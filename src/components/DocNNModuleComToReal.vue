...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / NN Module / __com_to_real__',
      functionName: '__com_to_real__',
      functionDesc: '将复数数据拆分并转换为神经网络可处理的实数形式。',
      callTitle: '调用方式',
      signature: '__com_to_real__(self, data)',
      paramsTitle: '参数',
      params: [
        { name: 'data', format: 'complex ndarray / tensor', description: '待转换的复数数据' }
      ],
      returnTitle: '返回值',
      returnParam: {
        name: 'real_data',
        format: 'tensor',
        description: '转换后的实数数据'
      }
    })
  }
})
