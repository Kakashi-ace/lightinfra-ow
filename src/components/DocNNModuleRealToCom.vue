...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / NN Module / __real_to_com__',
      functionName: '__real_to_com__',
      functionDesc: '将实数形式的数据恢复为复数形式，供光信号计算使用。',
      callTitle: '调用方式',
      signature: '__real_to_com__(self, data)',
      paramsTitle: '参数',
      params: [
        { name: 'data', format: 'ndarray / tensor', description: '实数形式的数据' }
      ],
      returnTitle: '返回值',
      returnParam: {
        name: 'complex_data',
        format: 'complex tensor',
        description: '转换后的复数数据'
      }
    })
  }
})
