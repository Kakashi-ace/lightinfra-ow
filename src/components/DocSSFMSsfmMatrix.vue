...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / SSFM Module / SSFM / __ssfm_matrix__',
      functionName: '__ssfm_matrix__',
      signature: '__ssfm_matrix__(self, sigin)',
      functionDesc: '执行SSFM算法求解双偏振信号传输方程。算法在每一步更新PMD、线性和非线性效应计算参数，并完成当前跨段的双偏振信号传输。',
      paramsTitle: '函数参数',
      params: [
        { name: 'sigin', format: 'list', description: '输入该跨段的双偏振信号' }
      ],
      returnTitle: '返回',
      returnParam: {
        name: 'sigout',
        format: 'list',
        description: '经过当前跨段传输得到的双偏振信号'
      }
    })
  }
})
