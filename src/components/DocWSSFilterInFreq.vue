...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / WSS / __filter_in_freq__',
      functionName: '__filter_in_freq__',
      signature: '__filter_in_freq__(self, sigin, transfer_func, data_mode=\'numpy\')',
      functionDesc: '在频域对输入信号进行滤波。将输入信号变换到频域后与滤波器传输函数相乘，再反变换得到时域输出。',
      paramsTitle: '函数参数',
      params: [
        { name: 'sigin', format: 'list', description: '待滤波的输入信号' },
        { name: 'transfer_func', format: 'tensor或ndarray', description: '滤波器频域传输函数' },
        { name: 'data_mode', format: 'str，可选', description: "运算数据类型，可取 {'numpy', 'tensor'}" }
      ],
      returnTitle: '返回',
      returnParam: {
        name: 'sigout',
        format: 'list',
        description: '完成频域滤波后的信号'
      }
    })
  }
})
