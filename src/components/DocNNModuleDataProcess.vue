...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / NN Module / __data_process__',
      functionName: '__data_process__',
      functionDesc: '对模型输入或输出数据进行标准化、维度整理等预处理。',
      callTitle: '调用方式',
      signature: '__data_process__(self, data)',
      paramsTitle: '参数',
      params: [
        { name: 'data', format: 'ndarray / tensor', description: '待处理的输入或输出数据' }
      ],
      returnTitle: '返回值',
      returnParam: {
        name: 'data_out',
        format: 'tensor',
        description: '完成预处理的数据'
      }
    })
  }
})
