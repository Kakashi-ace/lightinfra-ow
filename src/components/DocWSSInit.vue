...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / WSS / __init__',
      functionName: '__init__',
      signature: '__init__(self, filter_type, filter_config, wdm_data_mode=\'numpy\', dwdm_data_mode=\'numpy\', *args, **kwargs)',
      functionDesc: '初始化WSS类对象，传入滤波器类型、滤波器配置以及复用和解复用过程的数据类型，并调用init完成具体参数配置。',
      paramsTitle: '函数参数',
      params: [
        { name: 'filter_type', format: 'str', description: '复用或解复用过程使用的滤波器类型' },
        { name: 'filter_config', format: 'dict', description: '滤波器配置信息，包含\'type\'与\'args\'' },
        { name: 'wdm_data_mode', format: 'str，可选', description: '波分复用运算数据类型，默认\'numpy\'' },
        { name: 'dwdm_data_mode', format: 'str，可选', description: '解复用运算数据类型，默认\'numpy\'' },
        { name: '**kwargs', format: 'dict', description: '其它滤波器及通道配置参数' }
      ],
      returnTitle: '返回',
      returnDesc: '本函数无返回值。'
    })
