...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / NN Module / __nn_matrix__',
      functionName: '__nn_matrix__',
      functionDesc: '将输入数据组织为神经网络计算所需的矩阵形式。',
      callTitle: '调用方式',
      signature: '__nn_matrix__(self, data)',
      paramsTitle: '参数',
      params: [
        { name: 'data', format: 'ndarray / tensor', description: '待转换的输入数据' }
      ],
      returnTitle: '返回值',
      returnParam: {
        name: 'matrix',
        format: 'tensor',
        description: '用于模型计算的神经网络矩阵'
      }
    })
  }
})
