...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / SSFM Module / SSFM / __init__',
      functionName: '__init__',
      signature: '__init__(self, len_arr, beta0, beta1, beta2, beta3, pmd, alpha_loss, gamma, fft_num, sam_rate, infor_print=1, data_mode=\'numpy\', *args, **kwargs)',
      functionDesc: '基本初始化方法。传入基本参数。类的初始化过程分两阶段进行，该方法内将调用 init 进一步完成参数配置。',
      paramsTitle: '函数参数',
      params: [
        { name: 'len_arr', format: 'ndarray', description: '各个跨段的长度' },
        { name: 'beta0, beta1, beta2, beta3', format: 'float', description: '零阶至三阶色散系数' },
        { name: 'pmd', format: 'int', description: '取值0或1，决定双偏振信号仿真传输是否考虑PMD效应' },
        { name: 'alpha_loss', format: 'float', description: '链路损耗系数，单位Np/km' },
        { name: 'gamma', format: 'float', description: '非线性系数，单位 km⁻¹W⁻¹' },
        { name: 'sam_rate', format: 'int', description: '信道采样率，等于四倍的通道数乘以单通道符号速率，单位Gsam/s' },
        { name: 'infor_print', format: 'int，可选', description: '取值0或1，设定是否显示仿真传输进度，默认为1' },
        { name: 'data_mode', format: 'str，可选', description: "运算数据类型，可取 {'numpy', 'tensor'}，默认 'numpy'" },
        { name: '**kwargs', format: 'dict', description: "其它参数，包含 {'device', 'constant_pi', 'constant_h'}" }
      ],
      returnTitle: '返回',
      returnDesc: '本函数无返回值。'
    })
