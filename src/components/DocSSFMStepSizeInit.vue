...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / SSFM Module / Step_Size / __init__',
      functionName: '__init__',
      signature: '__init__(self, dz_mode, nl_gamma, nPol, pmd, alpha_loss, span_len, phi_max=0.005, total_step_num=1000, *args, **kwargs)',
      functionDesc: '初始化 Step_Size 类对象，传入步长计算模式、链路参数和双偏振仿真配置，并创建算法执行过程中所需的步长状态。',
      paramsTitle: '函数参数',
      params: [
        { name: 'dz_mode', format: 'str', description: "步长计算方法，可取 {'np', 'log', 'np_log', 'c'}" },
        { name: 'nl_gamma', format: 'float', description: '非线性系数' },
        { name: 'nPol', format: 'int', description: '偏振数目' },
        { name: 'pmd', format: 'int', description: '取值0或1，设定是否仿真PMD效应' },
        { name: 'alpha_loss', format: 'float', description: '链路损耗系数，单位Np/km' },
        { name: 'span_len', format: 'float', description: '当前跨段长度，单位km' },
        { name: 'phi_max', format: 'float，可选', description: '最大非线性相移，默认0.005 rad' },
        { name: 'total_step_num', format: 'int，可选', description: '常数步长法参考步数，默认1000' },
        { name: '**kwargs', format: 'dict', description: '其它步长及PMD计算配置参数' }
      ],
      returnTitle: '返回',
      returnDesc: '本函数无返回值。'
    })
