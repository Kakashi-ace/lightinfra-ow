    ...
    <main class="doc-content" id="doc-content">
      ...
      <section class="doc-section">
        ...
        <div class="declaration-content">
          <h3 class="declaration-subtitle">{{ docData.exceptionTitle }}</h3>
          <p class="declaration-text">{{ docData.exceptionDesc }}</p>
        </div>
      </section>
    </main>
...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / SSFM Module / SSFM calculation functions / linearity_matrix',
      functionName: 'linearity_matrix',
      signature: 'linearity_matrix(sigin, dz, phase_factor_freq, pmd, phase_factor_pmd_freq, data_mode=\'numpy\')',
      functionDesc: '计算双偏振信号传输过程中产生的线性效应，包括群速度色散；启用PMD时同时计算偏振模色散作用。计算在频域进行。',
      paramsTitle: '函数参数',
      params: [
        { name: 'sigin', format: 'list', description: '传输的双偏振信号' },
        { name: 'dz', format: 'float', description: '本次计算的色散效应作用距离' },
        { name: 'phase_factor_freq', format: 'tensor或ndarray', description: '色散引起的与频率相关的频域相移系数' },
        { name: 'pmd', format: 'int', description: '取值0或1，设定是否考虑并计算PMD效应' },
        { name: 'phase_factor_pmd_freq', format: 'tensor或ndarray', description: 'PMD效应对应的频域相移及偏振耦合系数' },
        { name: 'data_mode', format: 'str，可选', description: "运算数据类型，可取 {'numpy', 'tensor'}，默认 'numpy'" }
      ],
      returnTitle: '返回',
      returnParam: {
        name: 'sigout',
        format: 'list',
        description: '添加了该段线性效应的双偏振信号'
      },
      declarationTitle: '声明',
      exceptionTitle: 'AttributeError',
      exceptionDesc: '当运算数据类型参数 data_mode 的设置值不在可选范围内时触发："ERROR: data_mode is not defined as \'tensor\' or \'numpy\'"'
    })
  }
})
