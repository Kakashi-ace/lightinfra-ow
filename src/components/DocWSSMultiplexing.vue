...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / WSS / wdm_multiplexing',
      functionName: 'wdm_multiplexing',
      signature: 'wdm_multiplexing(self, sigin, para_update=1)',
      functionDesc: '执行波分复用。对各通道信号进行滤波和频移，并将多个波长通道合并为波分复用信号。',
      paramsTitle: '函数参数',
      params: [
        { name: 'sigin', format: 'list', description: '待复用的多通道信号' },
        { name: 'para_update', format: 'int，可选', description: '取值0或1，设定是否更新复用参数，默认1' }
      ],
      returnTitle: '返回',
      returnParam: {
        name: 'sigout',
        format: 'list',
        description: '完成波分复用后的信号'
      }
    })
  }
})
