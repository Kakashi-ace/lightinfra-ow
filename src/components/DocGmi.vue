    ...
    <main class="doc-main-content">
      ...
      <div class="doc-content">
        ...
        <div class="breadcrumb">
          ...
          <span class="separator">/</span>
          <span class="current">gmi</span>
        </div>

        <!-- 5.2 页面标题（函数名） -->
        <h1 class="page-title">gmi</h1>
        ...
        <p class="section-text">
          {{ docData?.description || '使用圆对称高斯噪声统计计算基于比特位的无记忆互信息之和，该量也称为广义互信息（GMI）。与基于符号的MI不同，对于二进制解码且解映射器和解码器之间无迭代的接收器，它是一个可实现的速率。' }}
        </p>
...
const functionSignature = "generalized_mutual_information_mc(llr, tx_bit, sym_map, probability, **kwargs)"

const parameters = [
  { name: 'llr', format: 'array_like', desc: '对数似然比' },
  { name: 'tx_bit', format: 'array_like', desc: '长度为N的数据序列，复数形式' },
  { name: 'sym_map', format: 'array_like', desc: '输入信号信号的星座图分布' },
  { name: 'probability', format: 'array_like', desc: '输入数据对应的概率分布' }
]

const returnValue = {
  name: 'gmi',
  format: 'float',
  desc: '基于高斯分布假设，计算得到的广义互信息'
}
