<template>
  <div class="base-optics-doc-page">
    <!-- 主内容区域 -->
    <main class="doc-main-content">
      ...
      <div class="doc-content">
        ...
        <div class="breadcrumb">
          ...
          <span class="separator">/</span>
          <span class="current">base_optics</span>
        </div>

        <!-- 5.2 页面标题 -->
        <h1 class="page-title">base_optics 类说明</h1>

        <!-- 5.3 页面副标题 -->
        <p class="page-subtitle">
          {{ docData?.subtitle || 'base_optics 是 IFTS 仿真工具中负责光学组件建模与信号生成的基础类。' }}
        </p>
        ...
        <section :id="pageNavItems[0].id" class="doc-section">
          ...
          <p class="section-text">
            {{ docData?.classDesc || '该类封装了光学器件和光信号的基本建模方法，包括激光器、调制器、光纤信道等组件的配置与信号生成。' }}
          </p>
        </section>
...
const attributes = [
  { name: 'data_mode', format: 'str', desc: '进行运算的数据类型，可设为 numpy 或 tensor，默认 numpy' },
  { name: 'name', format: 'str', desc: '对象名称' },
  { name: 'value', format: 'str', desc: '对象属性值' }
]

const methods = [
  { name: '__init__', desc: '初始化函数' },
  { name: 'data_mode_convert', desc: '在 numpy 与 tensor 数据格式之间转换' },
  { name: 'init_ftn', desc: '从配置中查找并返回函数句柄' },
  { name: 'init_obj', desc: '根据配置参数初始化对象实例' },
  { name: '__getarr__', desc: '获取对象属性' },
