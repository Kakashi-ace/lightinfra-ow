...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / EDFA / forward_pass',
      functionName: 'forward_pass',
      signature: 'forward_pass(self, sigin, rand_seed)',
      functionDescTitle: '函数说明',
      functionDesc: '信号通过 EDFA 进行放大。根据 mode 调用有噪声或无噪声的传输函数。',
      paramsTitle: '函数参数',
      params: [
      ...
      returnParam: {
        name: 'sigout',
        format: 'list',
        description: '经 EDFA 输出的信号'
      },
      declarationTitle: '声明',
      declaration: 'NotImplementedError 当 EDFA 模式参数 mode 不在可选范围内时触发。'
    })
  }
})
