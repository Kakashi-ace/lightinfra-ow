    ...
    <main class="doc-main-content">
      ...
      <div class="doc-content">
        ...
        <div class="breadcrumb">
          ...
          <span class="separator">/</span>
          <span class="current">ser</span>
        </div>

        <!-- 5.2 页面标题（函数名） -->
        <h1 class="page-title">ser</h1>
        ...
        <p class="section-text">
          {{ docData?.description || '根据发端数据和收端数据计算传输过程的误符号率（symbol error rate）' }}
        </p>
        ...
        <div class="table-container">
          ...
          <div class="table-divider"></div>
          <div v-for="(ret, index) in returnValues" :key="ret.name" class="table-row">
            <span class="table-col col1">{{ ret.name }}</span>
            <span class="table-col col2">{{ ret.format }}</span>
            <span class="table-col col3">{{ ret.desc }}</span>
          </div>
        </div>
      </div>
...
const functionSignature = "ser(x, y, receive_sym)"

const parameters = [
  { name: 'x', format: 'array_like', desc: '输入数据数组。是计算中的被比较数据，一般为发端符号序列' },
  { name: 'y', format: 'array_like', desc: '输入数据数组。是计算中的比较数据，一般为收端符号序列' },
  { name: 'receive_sym', format: 'array_like', desc: '接收到的符号序列' }
]

const returnValues = [
  { name: 'ser', format: 'float', desc: '误符号率' },
  { name: 'sym_error', format: 'array_like', desc: '传输错误的符号' },
  { name: 'sym_error_idx', format: 'array_like', desc: '传输错误符号的索引' }
]
