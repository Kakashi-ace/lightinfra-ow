    ...
    <main class="doc-main-content">
      ...
      <div class="doc-content">
        ...
        <div class="breadcrumb">
          ...
          <span class="separator">/</span>
          <span class="current">ber_awgn</span>
        </div>

        <!-- 5.2 页面标题（函数名） -->
        <h1 class="page-title">ber_awgn</h1>
        ...
        <p class="section-text">
          {{ docData?.description || '计算非编码AWGN信道下的误码率（BER)和误符号率（SER）' }}
        </p>
...
const functionSignature = "berawgn(EbNo, mode, M)"

const parameters = [
  { name: 'EbNo', format: 'float', desc: '噪声功率谱密度比的比特能量（以dB为单位）' },
  { name: 'mode', format: 'str', desc: '调制格式（QAM或者PAM）' },
  { name: 'M', format: 'int', desc: '调制阶数，必须是正整数' }
]

const returnValues = [
  { name: 'ber', format: 'float', desc: '误码率' },
