<template>
  <div class="doc-content-wrapper">
    ...
    <main class="doc-content" id="doc-content">
      ...
      <!-- 参考文献 -->
      <section class="doc-section" id="section-references">
        <h2 class="section-title">{{ docData.referencesTitle }}</h2>
        <p class="section-text reference-text">{{ docData.reference }}</p>
      </section>
    </main>
...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / coherent_receiver / forward_pass',
      functionName: 'forward_pass',
      signature: 'forward_pass(self, sigin, rand_seed)',
      functionDescTitle: '函数说明',
      functionDesc: '信号通过ICR模块，调用对象时自动调用该函数。注意：当前版本支持的模式仅为 \'naive\'。',
      paramsTitle: '函数参数',
      params: [
        { name: 'sigin', format: 'list', description: '经过信道传输，输入ICR信号' },
      ...
      returnParam: {
        name: 'x',
        format: 'list',
        description: '添加了接收机噪声的信号'
      },
      declarationTitle: '声明',
      declaration: 'NotImplementedError：当设定的ICR模式参数 mode 不在可选范围时触发。',
      referencesTitle: '参考文献',
      reference: 'Govind P. Agrawal. Fiber-Optic Communication Systems[M]. New Jersey: John Wiley & Sons, 2010: 305–307.'
    })
  }
})
