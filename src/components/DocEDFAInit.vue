...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / EDFA / __init__',
      functionName: '__init__',
      signature: "__init__(self, mode, nf_db, f_cut, gain, noise_bw, data_mode = 'numpy', **kwargs)",
      functionDescTitle: '函数说明',
      functionDesc: '传入 EDFA 模式及相关参数，初始化 EDFA 类对象，并完成放大器增益与 ASE 噪声参数配置。',
      paramsTitle: '函数参数',
      params: [
        { name: 'mode', format: 'str', description: "EDFA 模式，可取值 {'naive_pass', 'no_noise_pass'}" },
        { name: 'nf_db', format: 'float', description: 'EDFA 噪声系数，单位 dB' },
        { name: 'f_cut', format: 'float', description: '信号载波频率，单位 GHz' },
        { name: 'gain', format: 'float', description: 'EDFA 增益，线性倍数，无单位' },
        { name: 'noise_bw', format: 'float', description: 'ASE 噪声带宽，单位 GHz' },
        { name: 'data_mode', format: 'str，可选', description: "运算数据类型，可选值 'numpy' 或 'tensor'，默认 'numpy'" },
        { name: '**kwargs', format: 'dict', description: '其它参数，包括特殊常数、设备名称' }
      ]
    })
  }
})
