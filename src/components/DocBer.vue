    ...
    <main class="doc-main-content">
      ...
      <div class="doc-content">
        ...
        <div class="breadcrumb">
          ...
          <span class="separator">/</span>
          <span class="current">ber</span>
        </div>

        <!-- 5.2 页面标题（函数名） -->
        <h1 class="page-title">ber</h1>
        ...
        <p class="section-text">
          {{ docData?.description || '根据输入变量计算误码率（bit error rate）' }}
        </p>
...
const functionSignature = "ber(x, y, data_mode = 'numpy')"

const parameters = [
  { name: 'x', format: 'array_like', desc: '输入数据数组，一般为发端数据' },
  { name: 'y', format: 'array_like', desc: '输入数据数组，一般为收端数据' },
  { name: 'data_mode', format: 'str, optional', desc: '输入数据的格式（numpy数组或torch张量）' }
]

const returnValues = [
  { name: 'ber', format: 'float', desc: '计算得到的误码率' },
  { name: 'ber_per_num', format: 'float', desc: '计算得到的每比特误码率' }
]
