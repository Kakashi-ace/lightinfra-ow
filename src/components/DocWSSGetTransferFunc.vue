...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / WSS / __get_transfer_func__',
      functionName: '__get_transfer_func__',
      signature: '__get_transfer_func__(self, freq)',
      functionDesc: '根据滤波器类型、带宽与陡度配置计算滤波器的频域传输函数。',
      paramsTitle: '函数参数',
      params: [
        { name: 'freq', format: 'tensor或ndarray', description: '用于计算滤波器响应的频率轴' }
      ],
      returnTitle: '返回',
      returnParam: {
        name: 'transfer_func',
        format: 'tensor或ndarray',
        description: '滤波器频域传输函数'
      }
    })
  }
})
