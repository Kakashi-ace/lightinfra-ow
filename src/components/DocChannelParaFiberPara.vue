<template>
  <div class="channel-para-fiber-para-doc-page">
    <!-- 主内容区域 -->
    <main class="doc-main-content">
      ...
      <div class="doc-content">
        ...
        <div class="breadcrumb">
          ...
          <span class="separator">/</span>
          <span class="current">__fiber_para__</span>
        </div>

        <!-- 5.2 页面标题（函数名） -->
        <h1 class="page-title">__fiber_para__</h1>

        <!-- 5.3 函数签名框 -->
        <div class="function-signature">
          <span class="signature-text">__fiber_para__(self)</span>
        ...
        <p class="section-text">
          {{ docData?.description || '根据基本参数计算并配置仿真光纤信道参数，包括各跨段长度、传输损耗及各种信道效应的参数。' }}
        </p>
        ...
        <!-- 5.7 声明正文 -->
        <p class="section-text">
          {{ docData?.declaration || 'ValueError：当链路总长参数不等于设定的各跨段长度之和时触发：\'The sum of span length is different with total length !\'。' }}
        </p>
        ...
        <!-- 5.9 警告正文 -->
        <p class="section-text">
          {{ docData?.warning || '\'ch_para: SSFM is used\' 或 \'ch_para: NN is used\'' }}
        </p>
        ...
        <!-- 5.11 注意正文 -->
        <p class="section-text">
          {{ docData?.note || '输出提示当前使用的仿真光纤传输方法（SSFM或AI模型）。' }}
        </p>
      </div>
...
const handleCopy = () => {
  const code = '__fiber_para__(self)'
...
<style scoped>
.channel-para-fiber-para-doc-page {
  width: 100%;
  min-height: 2304px;
  background: #FFFFFF;
