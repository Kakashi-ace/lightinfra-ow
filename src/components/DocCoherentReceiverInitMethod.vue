...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / coherent_receiver / __init__',
      functionName: '__init__',
      signature: "__init__(self, mode, args, data_mode = 'numpy', **kwargs)",
      functionDescTitle: '函数说明',
      functionDesc: '传入配置信息及所需参数，初始化ICR类对象。内部调用 init 进行具体参数的配置。',
      paramsTitle: '函数参数',
      params: [
        { name: 'mode', format: 'str', description: "ICR模式，目前仅支持 'naive'" },
        { name: 'args', format: 'dict', description: 'ICR具体参数设置，传入 init 进行配置' },
        { name: 'data_mode', format: 'str，可选', description: "进行运算的数据类型，可选值 'numpy' 或 'tensor'，默认 'numpy'" },
