...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / SSFM Module / Step_Size / __nonlinear_phase_step__',
      functionName: '__nonlinear_phase_step__',
      signature: '__nonlinear_phase_step__(self, p)',
      functionDesc: '使用非线性相位限制方法计算当前步长，使每一步产生的最大非线性相移不超过设定值 phi_max。',
      paramsTitle: '函数参数',
      params: [
        { name: 'p', format: 'float', description: '当前信号平均功率' }
      ],
      returnTitle: '返回',
      returnParam: {
        name: 'dz_now',
        format: 'float',
        description: '非线性相位限制方法得到的步长，单位km'
      }
    })
  }
})
