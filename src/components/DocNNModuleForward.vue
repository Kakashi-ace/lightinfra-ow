...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / NN Module / forward_pass',
      functionName: 'forward_pass',
      functionDesc: '执行神经网络前向计算，完成输入信号的跨段传输建模。',
      callTitle: '调用方式',
      signature: 'forward_pass(self, signal)',
      paramsTitle: '参数',
      params: [
        { name: 'signal', format: 'Signal', description: '待处理的输入光信号' }
      ],
      returnTitle: '返回值',
      returnParam: {
        name: 'signal_out',
        format: 'Signal',
        description: '神经网络计算后的输出信号'
      }
    })
  }
})
