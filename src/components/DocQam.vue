    ...
    <main class="doc-main-content">
      ...
      <div class="doc-content">
        ...
        <div class="breadcrumb">
          ...
          <span class="separator">/</span>
          <span class="current">qam</span>
        </div>

        <!-- 5.2 页面标题（函数名） -->
        <h1 class="page-title">qam</h1>
        ...
        <p class="section-text">
          {{ docData?.description || '根据比特位数，获得正交振幅调制(QAM)信号星座图' }}
        </p>
        ...
        <div class="table-container">
          ...
          <div v-for="(param, index) in parameters" :key="param.name" class="table-row" :class="{ 'row-tall': index === 1 }">
        ...
        <div class="table-container">
          ...
          <div class="table-divider"></div>
          <div class="table-row row-tall">
            <span class="table-col col1">{{ declaration.param }}</span>
            <span class="table-col col2">{{ declaration.format }}</span>
            <span class="table-col col3">{{ declaration.desc }}</span>
          </div>
        </div>
      </div>
...
const functionSignature = "qam(M, gs = 0)"

const parameters = [
  { name: 'M', format: 'int', desc: '待调制信号的比特位数' },
  { name: 'gs', format: 'bool, optional', desc: '代表当前是否使用几何整形对信号进行调制（默认为0，当前版本不可用）' }
]
...
const declaration = {
  ...
  param: 'ValueError',
  format: 'Text',
  desc: "'does not support the gs constellations now' 当前版本不支持几何整形"
}
