...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / SSFM Module / Step_Size / __check_step_size__',
      functionName: '__check_step_size__',
      signature: '__check_step_size__(self, dz_now)',
      functionDesc: '检查并修正当前步长，保证累积传播距离不超过跨段长度；启用PMD时，同时根据PMD分段记录更新相关计算参数。',
      paramsTitle: '函数参数',
      params: [
        { name: 'dz_now', format: 'float', description: '当前待检查的步长，单位km' }
      ],
      returnTitle: '返回',
      returnParam: {
        name: 'dz_now',
        format: 'float',
        description: '检查并修正后的当前步长，单位km'
      }
    })
  }
})
