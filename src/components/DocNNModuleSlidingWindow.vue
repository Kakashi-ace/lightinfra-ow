...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / NN Module / __sliding_window__',
      functionName: '__sliding_window__',
      functionDesc: '按照指定窗口宽度和步长对序列数据执行滑动窗口切分。',
      callTitle: '调用方式',
      signature: '__sliding_window__(self, data, window_size, step=1)',
      paramsTitle: '参数',
      params: [
        { name: 'data', format: 'ndarray / tensor', description: '待切分的序列数据' },
        { name: 'window_size', format: 'int', description: '滑动窗口长度' },
        { name: 'step', format: 'int，可选', description: '窗口移动步长，默认值为 1' }
      ],
      returnTitle: '返回值',
      returnParam: {
        name: 'windows',
        format: 'tensor',
        description: '滑动窗口切分后的数据'
      }
    })
  }
})
