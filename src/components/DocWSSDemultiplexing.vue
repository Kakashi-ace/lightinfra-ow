...
const props = defineProps({
  docData: {
    type: Object,
    default: () => ({
      breadcrumb: '资源 / 文档 / IFTS / 代码说明文档 / Channel / WSS / wdm_demultiplexing',
      functionName: 'wdm_demultiplexing',
      signature: 'wdm_demultiplexing(self, sigin, para_update=1)',
      functionDesc: '执行波分解复用。选择目标波长通道并进行频移与滤波，输出各通道的基带信号。',
      paramsTitle: '函数参数',
      params: [
        { name: 'sigin', format: 'list', description: '待解复用的波分复用信号' },
        { name: 'para_update', format: 'int，可选', description: '取值0或1，设定是否更新解复用参数，默认1' }
      ],
      returnTitle: '返回',
      returnParam: {
        name: 'sigout',
        format: 'list',
        description: '完成波分解复用后的多通道信号'
      }
    })
  }
})
